...
public class IdempotenceFixture
{
    ...
    private void ConfigureServices(IServiceCollection services, string test, int threadCount, bool isTransactional, string queueUriFormat)
    {
        ...
        services
            .AddTransientQueues().AddTransactionScope(builder =>
            {
    ...
    protected async Task TestIdempotenceProcessingAsync(IServiceCollection services, bool isTransactional, bool enqueueUniqueMessages)
    {
        const string queueUriFormat = "transient-queue://./{0}";
        ...
        try
        {
            ...
            handleMessageObserver.HandlerException += (_, _) =>
            {
                exception = true;
            };
